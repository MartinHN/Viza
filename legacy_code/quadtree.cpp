...
QuadTree::QuadTree(float* inp_data, int N)
{
    ...
    float* mean_Y = new float[QT_NO_DIMS]; for(int d = 0; d < QT_NO_DIMS; d++) mean_Y[d] = .0;
    float*  min_Y = new float[QT_NO_DIMS]; for(int d = 0; d < QT_NO_DIMS; d++)  min_Y[d] =  DBL_MAX;
    float*  max_Y = new float[QT_NO_DIMS]; for(int d = 0; d < QT_NO_DIMS; d++)  max_Y[d] = -DBL_MAX;
    for(int n = 0; n < N; n++) {
    ...
    for(int d = 0; d < QT_NO_DIMS; d++) mean_Y[d] /= (float) N;
...
void QuadTree::print() 
{
    ...
    if(is_leaf) {
        ...
        for(int i = 0; i < size; i++) {
            float* point = data + index[i] * QT_NO_DIMS;
