...
void TSNE::run(float* X, int N, int D, float* Y, int no_dims, float perplexity, float theta) {
    ...
    float* P; int* row_P; int* col_P; float* val_P;
    if(exact) {
...
void TSNE::computeGradient(float* P, int* inp_row_P, int* inp_col_P, float* inp_val_P, float* Y, int N, int D, float* dC, float theta)
{
...
void TSNE::computeExactGradient(float* P, float* Y, int N, int D, float* dC) {
	...
	for(int n = 0; n < N; n++) {
    	for(int m = 0; m < N; m++) {
            if(n != m) {
                float mult = (P[n * N + m] - (Q[n * N + m] / sum_Q)) * Q[n * N + m];
                for(int d = 0; d < D; d++) {
...
float TSNE::evaluateError(float* P, float* Y, int N) {
...
float TSNE::evaluateError(int* row_P, int* col_P, float* val_P, float* Y, int N, float theta)
{
    
    // Get estimate of normalization term
    const int QT_NO_DIMS = 2;
    QuadTree* tree = new QuadTree(Y, N);
    float buff[QT_NO_DIMS] = {.0, .0};
    float sum_Q = .0;
...
void TSNE::computeGaussianPerplexity(float* X, int N, int D, int** _row_P, int** _col_P, float** _val_P, float perplexity, int K) {
...
void TSNE::computeGaussianPerplexity(float* X, int N, int D, int** _row_P, int** _col_P, float** _val_P, float perplexity, float threshold) {
    
    // Allocate some memory we need for computations
    float* buff  = (float*) malloc(D * sizeof(float));
    float* DD    = (float*) malloc(N * sizeof(float));
    float* cur_P = (float*) malloc(N * sizeof(float));
	...
	for(int n = 0; n < N; n++) {
        ...
        for(int m = 0; m < N; m++) {
            if(cur_P[m] > threshold / (float) N) total_count++;
        }
    }
    ...
    *_val_P = (float*) malloc(total_count * sizeof(float));
    int* row_P = *_row_P;
    int* col_P = *_col_P;
    float* val_P = *_val_P;
...
void TSNE::computeSquaredEuclideanDistance(float* X, int N, int D, float* DD) {
    ...
    cblas_sgemm(CblasColMajor, CblasTrans, CblasNoTrans, N, N, D, -2.0, X, D, X, D, 1.0, DD, N);
...
//bool TSNE::load_data(float** data, int* n, int* d, float* theta, float* perplexity) {
//	
...
//	*data = (float*) calloc(*d * *n, sizeof(float));
